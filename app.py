...
def _run_chat_sse(
    ...
    """POST /v1/chat/stream，按 SSE 增量更新 live_placeholder，返回落库用正文。"""
    url = f"{_sse_chat_base_url()}/v1/chat/stream"
    payload = {
    ...
    try:
        ...
        raise RuntimeError(
            f"无法连接 SSE 服务 {url}。若未使用内嵌服务，请先运行: "
...
with st.sidebar:
    ...
    st.caption(
        f"SSE：`{_sse_chat_base_url()}` /v1/chat/stream；"
